...
# Full credit to Prof. Weng-Keen Wong for outline/skeleton/framework code
...
# GLOBAL PARAMETERS FOR NETW5RK ARCHITECTURE
number_of_layers = 5
...
def main():
  ...
  # Load data and DO NOT display an example so you don't have to immediately close it
  ...
  for i in range(max_epochs):
    ...
    while j < len(X_train):
      ...
      Y_batch = Y_train[inds[j:j+b]].astype(np.int_)
  ...
  for i in range(0, len(predictions)):
    test_predictions.append(f"{i},{predictions[i][0]}")

  with open("test_predictions.csv", "w") as f:
    f.write("id,digit\n")
    f.write("\n".join(test_predictions))
  logging.info("Test predictions saved to test_predictions.csv")
...
# Credit to Prof. Weng-Keen Wong for following code
...
def evaluateValidation(model, X_val, Y_val, batch_size):
  ...
  while j < len(X_val):
    ...
    X_batch = X_val[j:j+b]
    Y_batch = Y_val[j:j+b].astype(np.int_)
...
def loadData(normalize = True):
  ...
  Y_train = train[:,-1].astype(np.int_)[:,np.newaxis]
  Y_val = val[:,-1].astype(np.int_)[:,np.newaxis]
